...
run_service() {
  ...
  if [ "$service" == "gateway/" ]; then
    ...
    mvn spring-boot:run -pl $service -Dreactor.netty.http.server.accessLogEnabled=true -Dspring-boot.run.jvmArguments="-Xmx176m -Xms176m" $args
  else
    mvn spring-boot:run -pl $service -Dspring-boot.run.jvmArguments="-Xmx176m -Xms176m" $args
  fi
}
...
if [ "$SERVICE_NAME" == "all" ]; then
  ...
  for module in $(mvn help:evaluate -Dexpression=project.modules -q -DforceStdout | sed -e 's/<[^>]*>//g' -e 's/\s*//g' | tr ',' '\n'); do
    if [ "$module" != "generator" ]; then
      echo "Running $module..."
      mvn spring-boot:run -pl $module -Dspring-boot.run.jvmArguments="-Xmx64m -Xms64m"
    fi
  done
else
  # 运行指定的单个服务，并传递额外参数
