...
public class DriverInfoController {
    ...
    public Result<Boolean> updateDriverAuthInfo(@RequestBody UpdateDriverAuthInfoForm update) {
        boolean flag = driverInfoService.updateDriverAuthInfo(update);
        return Result.ok(flag);
    }

    @Operation(summary = "创建司机人脸模型")
    @PostMapping("/creatDriverFaceModel")
    // DriverFaceModelForm 中以及封装好，把图片转成base64
    public Result<Boolean> creatDriverFaceModel(@RequestBody DriverFaceModelForm driverFaceModelForm) {
        Boolean flag = driverInfoService.creatDriverFaceModel(driverFaceModelForm);
        return Result.ok(flag);
    }
